// Code generated by go-swagger; DO NOT EDIT.

package build

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"
	"github.com/go-openapi/strfmt"

	"github.com/cldmnky/teamcity-rest-client-go/models"
)

// ReplaceCommentMultipleReader is a Reader for the ReplaceCommentMultiple structure.
type ReplaceCommentMultipleReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *ReplaceCommentMultipleReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {
	case 200:
		result := NewReplaceCommentMultipleOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		return nil, runtime.NewAPIError("unknown error", response, response.Code())
	}
}

// NewReplaceCommentMultipleOK creates a ReplaceCommentMultipleOK with default headers values
func NewReplaceCommentMultipleOK() *ReplaceCommentMultipleOK {
	return &ReplaceCommentMultipleOK{}
}

/*ReplaceCommentMultipleOK handles this case with default header values.

successful operation
*/
type ReplaceCommentMultipleOK struct {
	Payload *models.MultipleOperationResult
}

func (o *ReplaceCommentMultipleOK) Error() string {
	return fmt.Sprintf("[PUT /app/rest/builds/multiple/{buildLocator}/comment][%d] replaceCommentMultipleOK  %+v", 200, o.Payload)
}

func (o *ReplaceCommentMultipleOK) GetPayload() *models.MultipleOperationResult {
	return o.Payload
}

func (o *ReplaceCommentMultipleOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.MultipleOperationResult)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
